...
public class WebotsEnvironment extends EnvironmentImpl {
    ...
    private Supervisor designer;

    private Node targetObject;

    private Field targetObjectSize;
    ...
    private static final double WRIST_FORCE_THRESHOLD = 0.5;
    ...
    private static final double IR_CROSSED_AREA_UPPERBOUND_DISTANCE = 566.0; // 564 --> 566 for robust
    private static final double IR_CROSSED_AREA_LOWERBOUND_DISTANCE = 436.0; //438 --> 436 for robust; 390 for 10 times run
    ...
    private double left_gripper_pos_last, right_gripper_pos_last,
    arm1_pos_last,
    arm2_pos_last,
    arm3_pos_last,
    arm4_pos_last,
    arm5_pos_last,
    wheel1_base_pos_last,
    wheel2_base_pos_last,
    wheel3_base_pos_last;

    private static final double ARM_NOISE_STD = 0.01;

    @Override
    public void init() {

        designer = new Supervisor();

        //TODO: To switch between different target object by a paramters configured in XML file
        targetObject = designer.getFromDef("TallBox");
        //targetObject = designer.getFromDef("ShortBox");
        
        targetObjectSize = targetObject.getField("size");
        ...
        sensedData.put("beam_distance", 0.0);
        ...
        sensedData.put("arm1_pos", 0.0);
        sensedData.put("arm2_pos", 0.0);
        sensedData.put("arm3_pos", 0.0);
        sensedData.put("arm4_pos", 0.0);
        sensedData.put("arm5_pos", 0.0);
        sensedData.put("IR_Crossed", false);


        sensedData.put("targetObjectWidth", 0.0);

        sensedData.put("arm2_pos_real", 0.0);
        sensedData.put("arm3_pos_real", 0.0);
        sensedData.put("arm4_pos_real", 0.0);
	...
	double arm1_pos_tmp = 0.0, arm2_pos_tmp = 0.0, arm3_pos_tmp = 0.0, arm4_pos_tmp = 0.0,
                arm5_pos_tmp = 0.0;

        double[] objectSize = null;

        // It's an older variable because now arms position are always negative
        ...
        //0. Super Node supports the outside objects features directly
        objectSize = targetObjectSize.getSFVec3f();
        //System.out.println("[super node]the dims of tall box are: " + objectSize[0] + ", " + objectSize[1] + ", " + objectSize[2]);

        //Set z-dim of the object as its width
        //System.out.println("objectSize[2] is" + objectSize[2]);
        sensedData.put("targetObjectWidth", objectSize[2]);
        ...
        //1-2) beam distance
        sensedData.put("beam_distance", beam_Distance);
        ...
        arm1_pos_tmp = arm1.getPosition();
        arm2_pos_tmp = arm2.getPosition();
        arm3_pos_tmp = arm3.getPosition();
        arm4_pos_tmp = arm4.getPosition();
        arm5_pos_tmp = arm5.getPosition();

        sensedData.put("arm1_pos", arm1_pos_tmp);

        sensedData.put("arm2_pos", arm2_pos_tmp);
        sensedData.put("arm3_pos", arm3_pos_tmp);
        sensedData.put("arm4_pos", arm4_pos_tmp);
        
        sensedData.put("arm5_pos", arm5_pos_tmp);
        
        //11/12/2014 
        //No need to add noise in Env module,
        //but added noises in the Kalman Filter's formula
        
        //Create noise sensory data for the arm2~arm4 only
//        Random rng = new Random();
//        double arm2_pos_tmp_noise, arm3_pos_tmp_noise, arm4_pos_tmp_noise;
//
//        double dis_arm4, tmp_weight;        
        /*
         * An assumption that arm2 + arm3 + arm4 == PI does not changes even
         * when the noise is involved
         */
//        arm4_pos_tmp_noise = arm4_pos_tmp + ARM_NOISE_STD * rng.nextGaussian();
//        dis_arm4 = arm4_pos_tmp_noise - arm4_pos_tmp;
//        tmp_weight = rng.nextDouble();
//        arm2_pos_tmp_noise = arm2_pos_tmp - dis_arm4 * tmp_weight;
//        arm3_pos_tmp_noise = arm3_pos_tmp - dis_arm4 * (1.0 - tmp_weight);

//        sensedData.put("arm2_pos", arm2_pos_tmp_noise);
//        sensedData.put("arm3_pos", arm3_pos_tmp_noise);
//        sensedData.put("arm4_pos", arm4_pos_tmp_noise);
        ...
        if (arm4_force > WRIST_FORCE_THRESHOLD){
            System.out.println("Arm4_Forcce is: " + arm4_force);
            System.out.println("wristForce is actived !!");
        ...
        if ((leftIR_Distance > IR_CROSSED_AREA_LOWERBOUND_DISTANCE &&
            ...
            //System.out.println("IR_Crossed is True:" + leftIR_Distance + " : " + rightIR_Distance);
        } else {
            sensedData.put("IR_Crossed", false);
            //System.out.println("IR_Crossed is False:"  + leftIR_Distance + " : " + rightIR_Distance);
        }
