 ...
 * @author Daqi
 */
public class UncrashFSM extends ArmsFSMImpl{

    //Parameters for task running
    private static final Logger logger = Logger.getLogger(UncrashFSM.class.getCanonicalName());

    private int state;

    private static final int STATE_NIL = 0;
    private static final int STATE_CHECK = 1;
    private static final int STATE_HOP = 2;
    ...
    public UncrashFSM(int ticksPerRun) {
        super(ticksPerRun);
    }
    ...
    public void execute() {
        switch (state){
            ...
            case STATE_CHECK:
                ...
                if (wrist_force_active == true && toucher1_active == false && toucher2_active == false)
                    state = STATE_HOP;
                else{
                    state = STATE_NIL;
                }
                break;
            case STATE_HOP:
                //logger.log(Level.INFO, "STATE_HOP starts...");
                ...
                // rise_backward
                if (rise_backward() == false){
                    logger.log(Level.INFO, "rise_backward() got FALSE result!");
                    state = STATE_STUCK;
                    break;
                }
